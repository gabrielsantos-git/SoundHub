...
router.post('/upload', upload.array('arquivos'), async (req, res) => {
  ...
  try {
    ...
    // Verificar se tabela 'files' existe no banco de dados
    try {
      console.error('Verificando se tabela "files" existe no banco de dados...');
      ...
      if (tableError) {
        ...
        console.error('⚠️  A tabela "files" pode não existir no banco de dados');
        console.error('📝 Instruções:');
        console.error('   1. Acesse o Supabase Dashboard');
        console.error('   2. Vá em SQL Editor');
        console.error('   3. Execute o SQL para criar a tabela "files":');
        console.error('   CREATE TABLE files (');
        console.error('     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,');
        console.error('     nome TEXT NOT NULL,');
        console.error('     caminho TEXT NOT NULL,');
        console.error('     tipo TEXT,');
        console.error('     tamanho BIGINT,');
        console.error('     usuario_nome TEXT NOT NULL,');
        console.error('     descricao TEXT,');
        console.error('     status TEXT DEFAULT \'PENDING\',');
        console.error('     data_upload TIMESTAMP WITH TIME ZONE DEFAULT NOW()');
        console.error('   );');
        return res.status(500).json({ 
          error: 'Tabela "files" não existe no banco de dados. Crie a tabela manualmente no Supabase Dashboard.' 
        });
      } else {
        console.error('✅ Tabela "files" encontrada!');
      }
