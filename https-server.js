...
function generateSSLCertificates() {
  ...
  if (!fs.existsSync(certPath) || !fs.existsSync(keyPath)) {
    ...
    try {
      execSync('openssl req -x509 -newkey rsa:4096 -keyout server.key -out server.cert -days 365 -nodes -subj "/CN=localhost"', { cwd: __dirname });
      console.log('Certificados SSL gerados com sucesso!');
    } catch (error) {
      console.error('Erro ao gerar certificados SSL:', error.message);
      console.log('Tentando usar certificados existentes...');
    }
  }
}
...
app.get('/api/status', (req, res) => {
  res.json({ 
    status: 'online', 
    message: 'SoundHub funcionando em HTTPS',
...
server.on('connection', (socket) => {
  console.log('Nova conexão HTTPS estabelecida');
  
  socket.on('close', (hadError) => {
    console.log(`Conexão HTTPS fechada. Erro: ${hadError}`);
  });
  
  socket.on('error', (err) => {
    console.error('Erro na conexão HTTPS:', err.message);
  });
});
...
server.listen(PORT, () => {
  console.log(`Servidor SoundHub rodando em HTTPS na porta ${PORT}`);
  console.log(`Ambiente: ${process.env.NODE_ENV}`);
  console.log(`URL: https://localhost:${PORT}`);
  console.log(`A Window Management API agora está habilitada!`);
});
